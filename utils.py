...
def get_video_path(url, download_dir='downloads'):
    ...
    Handles various Facebook and Instagram URL formats and ensures the filename is valid.
    """
    ...
    # Instagram Pattern: /reel/ID
    if not video_id:
        insta_match = re.search(r'instagram\.com/reel/([^/?&#]+)', clean_url)
        if insta_match:
            video_id = f"ig_{insta_match.group(1)}"

    # If no specific pattern matched, create a hash of the URL
    if not video_id:
        url_hash = hashlib.md5(clean_url.encode('utf-8')).hexdigest()[:8]
        video_id = f"reel_{url_hash}"
